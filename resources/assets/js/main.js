...
(function () {
  ...
  //Layout Switcher (Compact/Wide)
  let layoutSwitcher = document.querySelector('.dropdown-layout-switcher');

  // Get layout from local storage or use config default
  let storedLayout =
    localStorage.getItem('templateCustomizer-' + templateName + '--contentLayout') || //if no template layout then use Customizer layout
    (window.templateCustomizer?.settings?.defaultContentLayout ?? 'wide'); //!if there is no Customizer then use default layout as wide

  // Get current active layout from HTML class
  const htmlElement = document.documentElement;
  const activeLayout = htmlElement.classList.contains('layout-compact') ? 'compact' : 'wide';

  // Set layout on click of layout switcher item if template customizer is enabled
  if (window.templateCustomizer && layoutSwitcher) {
    let layoutSwitcherItems = [].slice.call(layoutSwitcher.children[1].querySelectorAll('.dropdown-item'));
    layoutSwitcherItems.forEach(function (item) {
      item.classList.remove('active');
      item.addEventListener('click', function () {
        let currentLayout = this.getAttribute('data-layout');
        if (currentLayout === 'compact') {
          window.templateCustomizer.setContentLayout('compact');
          // Update HTML class
          htmlElement.classList.remove('layout-wide');
          htmlElement.classList.add('layout-compact');
          // Update icon
          const icon = layoutSwitcher.querySelector('i');
          icon.classList.remove('ti-layout-grid');
          icon.classList.add('ti-layout-list');
          // Update active state
          layoutSwitcherItems.forEach(i => i.classList.remove('active'));
          this.classList.add('active');
        } else if (currentLayout === 'wide') {
          window.templateCustomizer.setContentLayout('wide');
          // Update HTML class
          htmlElement.classList.remove('layout-compact');
          htmlElement.classList.add('layout-wide');
          // Update icon
          const icon = layoutSwitcher.querySelector('i');
          icon.classList.remove('ti-layout-list');
          icon.classList.add('ti-layout-grid');
          // Update active state
          layoutSwitcherItems.forEach(i => i.classList.remove('active'));
          this.classList.add('active');
        }
      ...
      if (item.getAttribute('data-layout') === activeLayout) {
        // Add 'active' class to the item if it matches the activeLayout
        item.classList.add('active');
      }
    });

    // Update layout switcher icon based on the stored layout
    const layoutSwitcherIcon = layoutSwitcher.querySelector('i');

    if (storedLayout === 'compact' || activeLayout === 'compact') {
      layoutSwitcherIcon.classList.add('ti-layout-compact');
      new bootstrap.Tooltip(layoutSwitcherIcon, {
        title: 'Layout Compact',
        fallbackPlacements: ['bottom']
      });
    } else {
      layoutSwitcherIcon.classList.add('ti-layout-grid');
      new bootstrap.Tooltip(layoutSwitcherIcon, {
        title: 'Layout Wide',
        fallbackPlacements: ['bottom']
      });
